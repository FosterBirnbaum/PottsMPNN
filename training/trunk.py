...
class FoldingTrunk(nn.Module):
    def __init__(self, **kwargs):
        ...
        if not self.cfg.use_weights:
            self.cfg.num_blocks = 2
            self.cfg.max_recycles = 1
            self.cfg.sequence_state_dim = 256
            self.cfg.pairwise_state_dim = 128
            self.cfg.chunk_size = 128
        ...
        if not self.cfg.use_weights:
            ...
            self.reshape_z = nn.Linear(400, c_z)
        ...
        self.node_dropout = nn.Dropout(self.cfg.node_dropout, inplace=False)
        self.edge_dropout = nn.Dropout(self.cfg.edge_dropout, inplace=False)
    ...
    def forward(self, seq_feats, pair_feats, true_aa, residx, mask, no_recycles: T.Optional[int] = None):
        ...
        if not self.cfg.use_weights:
            s_s_0 = self.reshape_s(seq_feats)
            s_z_0 = self.reshape_z(pair_feats)
        else:
            s_s_0 = seq_feats
            s_z_0 = pair_feats

        s_s_0 = self.node_dropout(s_s_0)
        s_z_0 = self.node_dropout(s_z_0)
        ...
        for recycle_idx in range(no_recycles):
            with ExitStack(): ## if recycle_idx == no_recycles - 1 else torch.no_grad() JFM
                # === Recycling ===
